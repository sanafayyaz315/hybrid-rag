...
class RagPipeline:
    ...
    def upload_file(
        ...
        texts = load_files(file_path_dir)
        ...
        logger.debug(f"{num_saved_parents} parent chunks saved at {self.parent_file_path}")
    ...
    def chat(
        ...
        if self.rewrite_query_prompt:
            ...
            query = json.loads(self.llm.invoke(messages))["query"]
        ...
        # 2. Similarity search
        hits = self.qstore.search(dense_query_vector=dense_query_embed, sparse_query_vector=sparse_query_embed, hybrid=True, top_k=50)
        ...
        if self.get_neighbors:
            neighbors = retrieve_parent_neighbors(ranks, parents, parent_chunks)
